...
void loop() {
  ...
  LED2_interval=0;
  for(int i=0;i<5;i++){
   LED1_interval=2000;//LED1_interval+map(analogRead(P1),0,1023,100,1000);
   LED2_interval=2000;//LED2_interval+map(analogRead(P2),0,1023,100,1000);
  }
  ...
  value=map(ms_from_start-ms_previous_read_LED1,0, LED1_interval, 0, 255);
  if (LED1_state==0) value=value; else value=255-value;
  ...
  value=map(ms_from_start-ms_previous_read_LED2,0, LED2_interval, 0, 255);
  if (LED2_state==0) value=255-value; else value=value;
